...
async fn create_tables()->Vec<i64>{
    ...
    for index in 0..5 {
        let response:Value  = client
            ...
            table_ids.push(response["id"].as_i64().expect("Mission or invalid id"));
    }
...
async fn create_menu()->Vec<i64>{
    let client = Client::new()
    let menu_names = vec!["Menu-01","Menu-02","Menu-03","Menu-04","Menu-05"];
    ...
    for  index in 0..5 {
        let response:Value=client
            ...
            menu_ids.push(response["id"].as_i64().expect("data is missing"));
...
async fn main() {
    ...
    let menu_ids = create_menu().await;
    println!("Hello, world!");

    //create client reqwest
    let client = Client::new();
